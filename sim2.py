...
def main(*args, **kwargs) :
	...
	for i in range(len(tj)):
		...
		carrier = A_c*np.cos(w_c*(n + np.random.normal(loc = 0, scale = tj[i], size=len(n))))
		...
		I_f = lfilter(b, a, carrier*np.sin(w_c*n))
		Q_f = lfilter(b, a, carrier*np.cos(w_c*n))

		# I_f = filt(I, 'box', 111)
		# Q_f = filt(Q, 'box', 111)

		phase = np.arctan(-I_f[:int(3e7)]/Q_f[:int(3e7)])	
